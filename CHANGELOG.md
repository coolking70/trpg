...
**P44a 局部战斗系统**（`src/data/skirmish.js` + `src/systems/SkirmishSystem.js`）：敌我各为数人小队的回合制连续战斗（沿用默认战斗伤害口径）。
- **援兵波次**：战损后按"战线 tide"（父战斗攻守强弱）决定援军是否及时补入——有利战线我方更易来援、敌方更难，这就是"战线有利/不利"的落地。
- **士气结局**：士气按"已投入兵力的战损比"动态计算（损约四成即逼近崩溃，援军/鼓舞回补）。结局不只全灭——**溃逃 / 投降 / 俘虏 / 上级鸣金撤退休整 / 全歼**，与士气强相关。
- 玩家以真实角色（HP 持续、结束回写）+ 数名袍泽参战；动作 斩击/据守/鼓舞/退却/生擒。

**P44b 战略桥接 + 局部时间放缓**：底层视角（soldier/officer）下其势力卷入战事（活跃围城/在途行军）时，getState 给"请缨参战"。参战据战线上下文（兵力对比定 tide）构建小队 + 援兵 + 小概率敌将。**关键：参战是被放大的瞬间，不调 `advanceSeason`、不推进战略季/旬**——局部时间放缓。`situation:'skirmish'` + 逐回合（auto 模式一战到底）。

**P44c 战功晋升 + 敌将偶遇重大事件**：累计战功 → 军衔晋升（士卒→什长→队率→屯长→军候）；**升至将官（军候）→ `playerRole` 转 `ruler`，从一卒擢升为可左右方略的指挥官（转入战略参与）**。个人英勇几乎不改全局——**唯独阵斩/生擒敌方关键将领**触发 `StrategicSystem.applyMajorEvent`：该势力民心/兵力受挫，若其正围攻我城则士气大挫、阵脚动摇而退兵——极少数个体撬动全局。

**验证**：jest 761/761（+skirmish 8 + skirmishBridge 6）、MCP 45/45、build 通过。deepseek-v4-flash 真 GM 抽样（西幻·步卒）：随队参战、援军/溃逃/斩将等结局如实呈现，战功累积、敌将偶遇推动战局。
