...
[![Tests](https://img.shields.io/badge/tests-706%2F706-brightgreen)](./__tests__)
...
- **学校系统（可选模块）** — 入学就读：选专业固定课 / 自选课程修满学分升级毕业；上课长属性·习技能，社团/校内外实践触发剧情，与师友/导师/校长/室友发展关系并于毕业时招募入队；校规惩罚（记过/留校察看/退学），考试·竞赛·跨校联赛（名次奖励，挂科留级退学）；魔法学院/武道馆/现代高中三套主题包，换皮只改 `schoolSchema` 数据
- **数据驱动可选模块** — 战略系统、学校系统均为可选模块（`preset.modules`），剧本无对应数据时全程 no-op、零耦合；MCP 三段管线据题材自动判定是否纳入
...
✅ **Phase 38–46**：战役级连战 / `main.js`↔`GameSession` 收敛 / 叙事化战争层（行军·情报·明暗·围城·救援）/ 战略主题抽象 `strategySchema` + 西幻·现代战争主题包 / 底层视角 `playerRole`（执棋者 or 棋中卒）/ 小兵实战参战 + 局部时间放缓 / 出身定制 + 事件身份门控（详见 CHANGELOG）
✅ **Phase 47**：战略系统解耦为**可选模块**（`preset.modules.strategy`，无数据 no-op）+ MCP 据 digest 自动判定是否纳入
✅ **Phase 48**：**学校系统（可选模块）**——选课/学分·学习成长·社团实践·师友招募·校规·考试竞赛跨校联赛；魔法学院/武道馆/现代高中三套主题包 + 示范剧本；`requireSchoolState` 校园剧情门控；MCP `recommendSchoolModule` 自动判定

🔮 后续可能方向：
- 学校系统深化（社团专属副本 / 同窗竞争对手 AI / 校园季节历法事件）
