...
蓝图 chapter 用 `legionBattlePlan` 编排**军团战**（与个人战 `combatPlan` 并列）：

```jsonc
legionBattlePlan: [{
  name: '官渡之战', battleType: 'field',          // field 野战 | siege 攻城 | defense 守城 | naval 水战
  supply: { player: 120, enemy: 80 },
  ourForces:   [{ unitType: 'cavalry', troops: 5000, generalRef: 'zhaoyun', formation: 'fengshi' },
                { unitType: 'archer',  troops: 4000, generalRef: 'liubei' }],
  enemyForces: [{ unitType: 'cavalry', troops: 4000, generalRef: 'caocao', machines: ['ballista'] }]
}]
```

- 兵种 `infantry/cavalry/archer/spearman/navy/siege`，克制（枪克骑、骑克弓…）；阵型受主将阵法等级解锁；器械 `catapult/ram/ballista/towerShip/mengchong` 按战型限带。
